...
variable "sql_server_name" {
  description = "The globally unique name of the SQL server. Defaults to sql-<deployment_name>-<environment>."
  type        = string
  default     = null
}

variable "sql_entra_admin_login_username" {
  description = "The login name of the Microsoft Entra ID administrator, e.g. a database administrators group."
  type        = string
}

variable "sql_entra_admin_object_id" {
  description = "The object ID of the Microsoft Entra ID administrator."
  type        = string
}
...
variable "database_sku" {
  description = "The SKU of the application database. The serverless default pauses when idle."
  type        = string
  default     = "GP_S_Gen5_1"
}

variable "database_auto_pause_delay_in_minutes" {
  description = "Minutes of inactivity before the serverless database pauses. Set null to disable pausing, or when database_sku is a provisioned SKU."
  type        = number
  default     = 60
}

variable "database_max_size_gb" {
  description = "The maximum size of the application database in GB."
  type        = number
  default     = 32
}

variable "database_zone_redundant" {
  description = "Whether the database is spread across availability zones. Requires a SKU and region that support zone redundancy."
  type        = bool
  default     = false
}
