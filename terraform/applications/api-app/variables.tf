...
variable "integration_subnet_name" {
  description = "The name of the delegated subnet used for the API's regional virtual network integration. Must match a key of the spoke's subnets variable."
  type        = string
  default     = "snet-api-integration"
}

variable "postgresql_subnet_name" {
  description = "The name of the application spoke subnet delegated to PostgreSQL flexible servers. Must match a key of the spoke's subnets variable."
  type        = string
...
variable "platform_key_vault_subscription_id" {
  description = "The Azure subscription holding the platform key vault this stack reads its secrets from, when it differs from the application spoke's. Leave null to resolve the vault in the application spoke's subscription (app_spoke_subscription_id), which itself defaults to this deployment's. The deployment identity needs read access to the vault there and role assignment write at its scope (e.g. Role Based Access Control Administrator), because this stack grants its runtime identity Key Vault Secrets User on the vault, on top of the data plane access the secrets themselves need."
  type        = string
  default     = null
}

variable "database_password_key_vault_secret" {
  description = "Reads the PostgreSQL administrator password from a secret pre-loaded into the spoke's platform key vault. The deployment agent must reach the vault's private data plane, and the running API resolves the same secret at runtime with its managed identity. key_vault_name and key_vault_resource_group_name default to the names the spoke derives, kv-<app_spoke_deployment_name>-<environment> and rg-<app_spoke_deployment_name>-<environment>-secrets; set them only when the spoke's platform_key_vault_name was overridden. The vault is resolved in platform_key_vault_subscription_id, defaulting to the application spoke's subscription."
  type = object({
...
variable "postgresql_server_name" {
  description = "The globally unique name of the PostgreSQL server. Defaults to psql-<deployment_name>-<environment>."
  type        = string
  default     = null
}

variable "postgresql_sku" {
  description = "The SKU of the PostgreSQL flexible server, e.g. B_Standard_B1ms for development and GP_Standard_D2ds_v5 for production."
  type        = string
  default     = "B_Standard_B1ms"
}

variable "postgresql_storage_mb" {
  description = "The storage of the PostgreSQL server in MB."
  type        = number
  default     = 32768
}

variable "postgresql_backup_retention_days" {
  description = "Days PostgreSQL backups are retained."
  type        = number
  default     = 7
}

variable "postgresql_high_availability" {
  description = "Zone-redundant or same-zone high availability for the PostgreSQL server. Leave null for a single server."
  type = object({
    mode                      = string
...
variable "postgresql_entra_administrator" {
  description = "The Microsoft Entra ID administrator of the PostgreSQL server, e.g. a database administrators group. Leave null to use the local administrator only."
  type = object({
    object_id      = string
...
variable "database_name" {
  description = "The name of the API's database."
  type        = string
  default     = "api"
}

variable "redis_name" {
  description = "The globally unique name of the Redis cache. Defaults to redis-<deployment_name>-<environment>."
  type        = string
  default     = null
}
...
variable "redis_family" {
  description = "The Redis family: C for Basic/Standard, P for Premium."
  type        = string
  default     = "C"
}

variable "redis_capacity" {
  description = "The size of the cache within its family: C0-C6 or P1-P5."
  type        = number
  default     = 1
}

variable "redis_zones" {
  description = "Availability zones of the Redis cache, e.g. [\"1\", \"2\", \"3\"]. Requires the Premium SKU. Leave null for a regional deployment."
  type        = list(string)
  default     = null
}

variable "app_configuration_name" {
  description = "The globally unique name of the App Configuration store. Defaults to appcs-<deployment_name>-<environment>."
  type        = string
  default     = null
}

variable "app_configuration_sku" {
  description = "The SKU of the App Configuration store: standard or premium."
  type        = string
  default     = "standard"
}
...
variable "app_service_sku" {
  description = "The SKU of the App Service plan, e.g. B1 for development and P1v3 for production."
  type        = string
  default     = "B1"
}

variable "app_service_worker_count" {
  description = "The number of App Service plan workers. Zone balancing needs at least two."
  type        = number
  default     = 1
}

variable "app_service_zone_balancing_enabled" {
  description = "Whether App Service plan workers spread across availability zones. Requires a Premium SKU and at least two workers."
  type        = bool
  default     = false
}
