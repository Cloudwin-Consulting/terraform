...
variable "integration_subnet_name" {
  description = "The name of the delegated subnet used for the consumer function's regional virtual network integration. Must match a key of the spoke's subnets variable."
  type        = string
  default     = "snet-eventpipe-integration"
}
...
variable "event_hub_namespace_name" {
  description = "The globally unique name of the Event Hubs namespace. Defaults to evhns-<deployment_name>-<environment>."
  type        = string
  default     = null
}

variable "event_hub_sku" {
  description = "The SKU of the Event Hubs namespace: Standard or Premium."
  type        = string
  default     = "Standard"
}
...
variable "event_hub_auto_inflate_enabled" {
  description = "Whether Standard throughput units scale up automatically. Requires event_hub_maximum_throughput_units."
  type        = bool
  default     = false
}
...
variable "event_hub_partition_count" {
  description = "The number of partitions of the telemetry event hub. Sets the consumer's maximum parallelism and cannot be reduced later."
  type        = number
  default     = 4
}

variable "event_hub_message_retention" {
  description = "Days telemetry events are retained in the event hub."
  type        = number
  default     = 1
}

variable "cosmos_account_name" {
  description = "The globally unique name of the Cosmos DB account. Defaults to cosno-<deployment_name>-<environment>."
  type        = string
  default     = null
}

variable "cosmos_database_name" {
  description = "The name of the Cosmos DB database holding the materialised view."
  type        = string
  default     = "telemetry"
}

variable "cosmos_container_name" {
  description = "The name of the Cosmos DB container the consumer writes the materialised view into."
  type        = string
  default     = "events"
}

variable "cosmos_zone_redundant" {
  description = "Whether the Cosmos DB primary region is spread across availability zones."
  type        = bool
  default     = false
}

variable "cosmos_automatic_failover_enabled" {
  description = "Whether the Cosmos DB account fails over automatically. Requires at least one cosmos_additional_geo_locations entry."
  type        = bool
  default     = false
}

variable "cosmos_additional_geo_locations" {
  description = "Secondary regions the Cosmos DB account replicates to, in failover priority order."
  type = list(object({
    location       = string
...
variable "function_app_name" {
  description = "The globally unique name of the consumer function app. Defaults to func-<deployment_name>-<environment>."
  type        = string
  default     = null
}

variable "function_app_sku" {
  description = "The SKU of the consumer's App Service plan, e.g. EP1."
  type        = string
  default     = "EP1"
}

variable "function_app_worker_count" {
  description = "The number of plan workers. Zone balancing needs at least two."
  type        = number
  default     = 1
}

variable "function_app_zone_balancing_enabled" {
  description = "Whether plan workers spread across availability zones. Requires a zone-capable SKU and at least two workers."
  type        = bool
  default     = false
}

variable "storage_account_name" {
  description = "The globally unique name of the function's host storage account. Defaults to st<deployment_name>-<environment> stripped of hyphens, which must stay within the 24 character storage account name limit."
  type        = string
  default     = null
}
