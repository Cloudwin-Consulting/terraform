...
variable "integration_subnet_name" {
  description = "The name of the delegated subnet used for the CMS's regional virtual network integration. Must match a key of the spoke's subnets variable."
  type        = string
  default     = "snet-cms-integration"
}

variable "mysql_subnet_name" {
  description = "The name of the application spoke subnet delegated to MySQL flexible servers. Must match a key of the spoke's subnets variable."
  type        = string
  default     = "snet-mysql"
}
...
variable "mysql_server_name" {
  description = "The globally unique name of the MySQL server. Defaults to mysql-<deployment_name>-<environment>."
  type        = string
  default     = null
}

variable "mysql_sku" {
  description = "The SKU of the MySQL flexible server, e.g. B_Standard_B1ms for development and GP_Standard_D2ds_v4 for production."
  type        = string
  default     = "B_Standard_B1ms"
}

variable "mysql_backup_retention_days" {
  description = "Days MySQL backups are retained."
  type        = number
  default     = 7
}

variable "mysql_high_availability" {
  description = "Zone-redundant or same-zone high availability for the MySQL server. Leave null for a single server."
  type = object({
    mode                      = string
...
variable "database_name" {
  description = "The name of the CMS database."
  type        = string
  default     = "cms"
}

variable "media_storage_account_name" {
  description = "The globally unique name of the media storage account. Defaults to st<deployment_name>-<environment> stripped of hyphens, which must stay within the 24 character storage account name limit."
  type        = string
  default     = null
}
