...
# Virtual machines
#
# QA mirrors production's shape at a smaller scale: the web tier is
# doubled up across two zones so failover is exercised before
# production, while the application tier stays single.
# ------------------------------------------------------------
linux_virtual_machine_count   = 2
windows_virtual_machine_count = 1
availability_zones            = ["1", "2"]

linux_virtual_machine_size   = "Standard_D2s_v5"
windows_virtual_machine_size = "Standard_D2s_v5"
...
# Backup
#
# QA proves the backup path works before production depends on it,
# keeping restore points for a week and staying in-region.
# ------------------------------------------------------------
enable_backup                         = true
backup_private_endpoint_dns_zone_name = "privatelink.uks.backup.windowsazure.com"
backup_storage_mode_type              = "ZoneRedundant"
backup_daily_retention_days           = 7

# ------------------------------------------------------------
# Azure SQL
#
# One server, on a provisioned SKU so query performance is comparable
# with production rather than with a database that pauses when idle.
# ------------------------------------------------------------
sql_server_count = 1
